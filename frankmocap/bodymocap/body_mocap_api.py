...
from frankmocap.bodymocap.utils.imutils import crop, crop_bboxInfo, process_image_bbox, process_image_keypoints, bbox_from_keypoints
...
import frankmocap.mocap_utils.geometry_utils_torch as gut
...
class BodyMocap(object):
    ...
    def regress(self, img_original, body_bbox_list):
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                # model forwardforward_call(*input, **kwarg
                #import pdb; pdb.set_trace()
                pred_pose_rot6d, pred_rotmat, pred_betas, pred_camera = self.model_regressor(norm_img.to(self.device))
                ...
                # pred_tf_mat_tensor = gut.rtvec_to_pose(pred_pose_rot6d.detach().cpu()[0])
                # pred_output['pred_tf_mat'] = pred_tf_mat_tensor.cpu().numpy().reshape(24, 4, 4) # (24, 4, 4)
